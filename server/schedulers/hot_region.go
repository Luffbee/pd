...
func (bs *balanceSolver) getHotPeers() []*statistics.HotPeerStat {
	...
	sort.Slice(byteSort, func(i, j int) bool {
		return byteSort[i].GetBytesRate() > byteSort[j].GetBytesRate()
	})
...
func (bs *balanceSolver) calcProgressiveRank() int64 {
	...
	if bs.rwTy == write && bs.opTy == transferLeader {
		...
		keyNotZero := true //peer.GetKeysRate() >= 10
		byteDecRatio := (dstLd.ByteRate + peer.GetBytesRate()) / (srcLd.ByteRate + 1)
		byteNotZero := true //peer.GetBytesRate() >= 100
		switch {
...
func (bs *balanceSolver) betterThan(old *solution) bool {
	...
	if bs.cur.srcPeerStat != old.srcPeerStat {
		...
		if bs.rwTy == write && bs.opTy == transferLeader {
			switch {
			case bs.cur.srcPeerStat.GetKeysRate() > old.srcPeerStat.GetKeysRate():
				return true
			case bs.cur.srcPeerStat.GetKeysRate() < old.srcPeerStat.GetKeysRate():
				return false
			}
		} else {
			byteRkCmp := rankCmp(bs.cur.srcPeerStat.GetBytesRate(), old.srcPeerStat.GetBytesRate(), stepRank(0, 100))
			keyRkCmp := rankCmp(bs.cur.srcPeerStat.GetKeysRate(), old.srcPeerStat.GetKeysRate(), stepRank(0, 10))
